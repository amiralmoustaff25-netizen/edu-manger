                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        ...
                        <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                            @foreach($logs as $log)
                            <tr class="{{ $log->status === 'failed' ? 'bg-red-50 dark:bg-red-900/20' : '' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($log->status === 'success')
                                        <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300">Réussie</span>
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    @can('voir-detail-log-connexion', $log)
                                        <a href="{{ route('login-logs.show', $log) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900 dark:hover:text-indigo-300">Détails</a>
                                    @endcan
                                </td>
                            </tr>
                            @endforeach
