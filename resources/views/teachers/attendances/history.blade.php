                    ...
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                        ...
                        <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
                            @forelse($attendances as $attendance)
                                <tr class="text-sm text-gray-700 dark:text-gray-200"><td class="whitespace-nowrap px-5 py-4">{{ $attendance->date->format('d/m/Y') }}</td><td class="px-5 py-4">{{ $attendance->classroom->name }}</td><td class="px-5 py-4">{{ $attendance->student->name }}</td><td class="px-5 py-4"><span class="rounded-full px-2.5 py-1 text-xs font-medium {{ $attendance->status === 'present' ? 'bg-green-100 text-green-700' : ($attendance->status === 'absent' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700') }}">{{ $attendance->status_label }}</span></td><td class="px-5 py-4">{{ $attendance->notes ?: '—' }}</td></tr>
                            @empty
