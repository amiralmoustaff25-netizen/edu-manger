    ...
    @if($currentYear)
        <div class="flex items-center gap-2 px-4 py-2 text-sm">
            ...
            <form method="POST" action="{{ route('context.school-year.update') }}" class="inline"
                ...
                @csrf
                <select name="school_year_id" class="border-0 bg-transparent font-semibold text-indigo-600 dark:text-indigo-400 focus:ring-1 focus:ring-indigo-500 rounded py-0.5 pr-6">
