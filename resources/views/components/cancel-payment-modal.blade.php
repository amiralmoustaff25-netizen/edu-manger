...
>
    ...
    <div class="relative w-full max-w-lg rounded-xl bg-white p-6 shadow-2xl dark:bg-slate-800" x-on:keydown.escape.window="open = false">
        ...
        <form method="POST" :action="action" class="mt-4 space-y-3">
            ...
            @method('PATCH')
            <div>
                <label for="cancellation_reason" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Motif de l'annulation (obligatoire)</label>
                <textarea name="cancellation_reason" id="cancellation_reason" x-model="reason" required rows="3" class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-slate-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500"></textarea>
            </div>
