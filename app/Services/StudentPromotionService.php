...
class StudentPromotionService
{
    ...
    private function suggest(?Classroom $classroom, ?SchoolYear $activeYear): array
    {
        ...
        // Priorité à la correspondance explicite configurée sur la classe
        // (promotes_to_classroom_id, ex. "CM1 A" -> "CM2 A") : plus fiable qu'une
        // déduction par simple ordre de niveau quand plusieurs sections parallèles
        // existent (CM1 A / CM1 B), qu'un ordre seul ne permet pas de désambiguïser.
        if ($classroom->promotes_to_classroom_id) {
            $explicit = Classroom::where('id', $classroom->promotes_to_classroom_id)
                ->where('school_year_id', $activeYear->id)
                ->first();

            if ($explicit) {
                return ['action' => 'promote', 'classroom_id' => $explicit->id, 'classroom_name' => $explicit->name];
            }
        }

        if ($classroom->ordre === null) {
            return ['action' => 'manual', 'classroom_id' => null, 'classroom_name' => null];
        }
